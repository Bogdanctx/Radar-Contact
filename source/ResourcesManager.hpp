...
public:
    ...
    const std::vector<float>& getRegionBox();
    const std::unordered_map<std::string, std::pair<int, int>>& getRegionAirports() const;
    const std::vector<std::pair<float, float>>& getWeatherTiles() const;
